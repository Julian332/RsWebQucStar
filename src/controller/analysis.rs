...
use crate::controller::analysis::IUniswapV2Pair::Swap;
...
pub async fn analysis_addr(Path(addr): Path<String>) -> Result<Json<AnalysisResp>, String> {
    ...
    Ok(Json(AnalysisResp {
        ...
        win_rate: if BigDecimal::from(tokens.len() as i64) == BigDecimal::from(0) {
            BigDecimal::from(0)
        } else {
            BigDecimal::from(win_count).with_scale(6) / BigDecimal::from(tokens.len() as i64)
        },
        tokens: tokens
            .into_iter()
            .map(|x| (x.0.to_string(), (x.1 .0, x.1 .1)))
            .collect(),
    }))
}
...
pub async fn subscribe_addr(
    ...
    let count: i64 = addr_subscribes
        .filter(following_addr.eq(addr.as_str()))
        .count()
    ...
    let new_addr_subscribes = insert_into(addr_subscribes)
        .values(new_addr_subscribes)
        .returning(AddrSubscribes::as_returning())
        .get_result(&mut connection)
        .unwrap();
...
pub async fn listen_and_send(pool: Pool<ConnectionManager<PgConnection>>) {
    ...
    while let Some(logs) = stream.next().await {
        let subscribes: Vec<AddrSubscribes> = addr_subscribes
            .filter(deleted.eq(false))
            .select(AddrSubscribes::as_select())
            .get_results(&mut connection)
            .unwrap();
        let subscribes: HashSet<_> = subscribes
            .iter()
            .map(|x| Address::from_hex(&x.following_addr).unwrap())
            .collect();
        for log in logs {
